...
Tag images with the Horus version for reproducibility:
```
docker build -t horus:$(python -c "from horus import __version__; print(__version__)") .
```
This produces `horus:0.8.0`, `horus:0.9.0`, etc. — always in sync with code.
`latest` always points to the newest build.

Run a specific version:
```
docker run -d --name horus -p 8080:8080 -v horus-data:/app/state horus:0.8.0
```

Switch versions (DB persists via named volume):
```
docker stop horus && docker rm horus
docker run -d --name horus -p 8080:8080 -v horus-data:/app/state horus:0.9.0
```

Clean DB (start fresh):
```
docker volume rm horus-data && docker volume create horus-data
```
